...
import { Grid, Link, List, ListItem, Typography, Card, Button, TextField, 
...
export default function ProductScreen(props) {
  ...
  const addToCartHandler = async () => {
    ...
    const quantity = existItem? existItem.quantity + 1 : 1;
    const { data } = await axios.get(`/api/products/${product._id}`);
    if(existItem && data.countInStock < quantity){
      window.alert('This product is already in your Cart with all avaliable Quantity. Continue shoping... or go to Cart for check-out')
      return;
  ...
  return (
    <Layout title={product.name} description={product.description}>
      <div className={classes.section}>
        <NextLink href="/" passHref>
          <Link><Typography className={classes.backPageLink}>Back to Home Page</Typography></Link>
        </NextLink>
      </div>
      <Grid container spacing={1}>
        <Grid item md={6} xs={12}>
        ...
        <Grid item md={3} xs={12}>
          <List>
            <ListItem><Typography component='h1'  className={classes.pname}>{product.name}</Typography></ListItem>
            ...
            <ListItem><Rating value={product.rating} readOnly></Rating>
              <Link href="#reviews">
                <Typography>({product.numReviews} reviews)</Typography>
              </Link></ListItem>
